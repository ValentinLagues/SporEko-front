            ...
            {allOffers.map((offer: IOffer, index: number) => {
                ...
                <div className='allOffers__offer' key={index}>
                    <ul className='allOffers__offer__detail'>
                        ...
                        <Link to={`/offers/${offer.id_offer}`} className="allOffers__offer__detail__linkOfferDetails">
                        <img src={offer.picture1} alt={`Main Picture of ${offer.id_user_seller}`} />
                        </Link>
                        </li>
                        <li className='allOffers__offer__detail__price'><strong>{offer.price} €</strong></li>
                        <li className='allOffers__offer__detail__fav'> 
                        {isFavorite.includes(index) ? <AiFillHeart className='inputIconFull' onClick={() => addFavorite({ offer, index })} /> : <AiOutlineHeart className='inputIconEmpty' onClick={() => addFavorite({ offer, index })} />}
                        </li>
